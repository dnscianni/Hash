...
/**
 * The RotationalHashTable uses a XOR-shift hash function to hash the keys in
 * order to place their corresponding value into the array of LinkedLists. It
 * also uses quickSort to sort the array of values.
 * 
 * @author David Scianni
 * 
 */
public class RotationalHashTable<K extends Comparable<K>, V> implements
		HashTable<K, V> {
	...
	/**
	 * the hash method will take in a key and use XOR-shift hashing to hash the
	 * key and will multiply it by -1 if it turns out to be negative. Lastly it
	 * will mod the hash with the tableSize, and return the hash.
	 * 
	 * @param key
	...
	private int hash(K key) {
		...
		for (int i = 0; i < keyString.length(); i++) {
			hash ^= (hash << 4) ^ (hash >> 10) ^ keyString.charAt(i);
		}
		if (hash < 0)
			hash *= -1;
